...
from models.dg_model import ResNet
from models import resnet_ttt
from utils.utils import get_source_target_names
...
def get_model(cfg, num_classes, device=None, dg_target=0):


    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        
    if cfg.DG.DATASET in {"domainnet", "PACS", "VLCS", "office-home", "terraincognita"} and cfg.TASK == "DG":
        _, target_domains_name = get_source_target_names(cfg)
        if cfg.DG.STAGE == "train":
            if cfg.MODEL.ARCH == "resnet":
                base_model = ResNet("resnet50", num_classes=num_classes)
        else:
            if cfg.MODEL.ARCH == "resnet":
                checkpoint_path = os.path.join(cfg.CKPT_DIR, cfg.DG.DATASET, f'{cfg.MODEL.ARCH}_{cfg.DG.DATASET}_{target_domains_name[dg_target]}_valid_model.pt')
                base_model = ResNet("resnet50", num_classes=num_classes)
                # state_dict = torch.load("/nas_data/WTY/project/TSD/code/train_output/PACS_art_painting_valid_model.pkl")
                # # Remove keys that start with 'network'
                # filtered_state_dict = {k: v for k, v in state_dict['model_dict'].items() if not k.startswith('network')}
                # state_dict['model_dict'] = filtered_state_dict
                # base_model.load_state_dict(state_dict['model_dict'], strict=True)
                base_model.load_state_dict(torch.load(checkpoint_path, map_location="cpu")['model_dict'], strict=True)

    elif cfg.MODEL.ADAPTATION == 'fata':
        base_model = resnet_ttt.get_resnet50_fata()
    else:
        try:
            ...
            try:
                if cfg.MODEL.ARCH == "vit_base_patch16_224":
                    # IMPORTANT: Replace with your actual ViT checkpoint path
                    # Example: npz_path = os.path.join(cfg.CKPT_DIR, "imagenet", "B_16-i21k-300ep-lr_0.001-aug_medium1-wd_0.1-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.01-res_224.npz")
                    npz_path = '/path/to/your/vit_checkpoint.npz'
                    base_model = timm.create_model('vit_base_patch16_224', pretrained=False)
                    _load_weights(base_model, npz_path)
                    base_model = normalize_model(base_model, mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
                else:
                ...
                try:
                    ...
                    if cfg.MODEL.ARCH == "resnet26":
                        base_model = resnet26.resnet(num_classes=num_classes, depth=26)
                        dataset = cfg.CORRUPTION.DATASET.split("_")[0]
                        checkpoint = torch.load(os.path.join(cfg.CKPT_DIR, dataset, f"rn26_{dataset}.pth"), map_location="cpu")
