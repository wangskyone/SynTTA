...
from utils.utils import get_accuracy, eval_domain_dict, draw_max_probs, draw_drift
...
from methods.syntta import SynTTA



# wild TTA = reset_each_shift_correlated
# practical TTA = correlated
# continual TTA = continual
...
def evaluate(description):
    ...
    if cfg.CORRUPTION.DATASET in {"domainnet126"}:
        ...
        if "correlated" in cfg.SETTING:
            dom_names_all = dom_names_all = [ "motion_blur", "snow", "fog", "shot_noise", 
                                              "defocus_blur", "contrast", "zoom_blur", "brightness", "frost",
                                              "elastic_transform",  "glass_blur", "gaussian_noise", "pixelate",
                                                "jpeg_compression", "impulse_noise", 
                             ]


            # dom_names_all = ["motion_blur"]
    ...
    logger.info(f"Evaluating {cfg.CORRUPTION.DATASET} with {cfg.MODEL.ADAPTATION} adaptation method")
    errs, errs_5, domain_dict = evaluate_corruption(cfg, model, dom_names_loop, severities, dom_names_all)


def evaluate_corruption(cfg, model, dom_names_loop, severities, dom_names_all):
    errs = []
    errs_5 = []
    domain_dict = {}
    ...
    for i_dom, domain_name in enumerate(dom_names_loop):
        if i_dom == 0 or "reset_each_shift" in cfg.SETTING:
            try:
                ...
                logger.warning(f"Model does not support reset: {e}")
        ...
        for severity in severities:
            ...
            # evaluate the model
            acc, domain_dict, domain_acc_list = get_accuracy(model,
    ...
    return errs, errs_5, domain_dict
